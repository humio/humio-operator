...
const (
	nginxIngressProxyBodySize     = "512m"
	nginxIngressProxyHTTPVersion  = "1.1"
	nginxIngressConfigFlagEnabled = "true"
)
...
more_set_headers "X-XSS-Protection: 1; mode=block";`
	...
	annotations["nginx.ingress.kubernetes.io/enable-cors"] = nginxIngressConfigFlagEnabled
	annotations["nginx.ingress.kubernetes.io/upstream-vhost"] = hostname

	if ingressTLSOrDefault(hc) {
		annotations["nginx.ingress.kubernetes.io/force-ssl-redirect"] = nginxIngressConfigFlagEnabled
	}
...
func ConstructGeneralIngress(hc *humiov1alpha1.HumioCluster, hostname string) *networkingv1.Ingress {
	annotations := make(map[string]string)
	annotations["nginx.ingress.kubernetes.io/proxy-body-size"] = nginxIngressProxyBodySize
	annotations["nginx.ingress.kubernetes.io/proxy-http-version"] = nginxIngressProxyHTTPVersion
...
func ConstructStreamingQueryIngress(hc *humiov1alpha1.HumioCluster, hostname string) *networkingv1.Ingress {
	...
	annotations["nginx.ingress.kubernetes.io/use-regex"] = nginxIngressConfigFlagEnabled
