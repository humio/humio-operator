...
`
			...
			Eventually(func() error {
				_, err := kubernetes.GetConfigMap(ctx, k8sClient, controller.ViewGroupPermissionsConfigMapName(toCreate), toCreate.Namespace)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			for _, pod := range clusterPods {
				...
				Expect(pod.Spec.Volumes).To(ContainElement(corev1.Volume{
					...
					VolumeSource: corev1.VolumeSource{
						ConfigMap: &corev1.ConfigMapVolumeSource{
							LocalObjectReference: corev1.LocalObjectReference{
								Name: controller.ViewGroupPermissionsConfigMapName(toCreate),
							},
							DefaultMode: &mode,
						},
					},
			...
			configMap, _ := kubernetes.GetConfigMap(ctx, k8sClient, controller.ViewGroupPermissionsConfigMapName(toCreate), key.Namespace)
...
`
			...
			suite.UsingClusterBy(key.Name, "Confirming config map was created")
			Eventually(func() error {
				_, err := kubernetes.GetConfigMap(ctx, k8sClient, controller.RolePermissionsConfigMapName(toCreate), toCreate.Namespace)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			for _, pod := range clusterPods {
				...
				Expect(pod.Spec.Volumes).To(ContainElement(corev1.Volume{
					...
					VolumeSource: corev1.VolumeSource{
						ConfigMap: &corev1.ConfigMapVolumeSource{
							LocalObjectReference: corev1.LocalObjectReference{
								Name: controller.RolePermissionsConfigMapName(toCreate),
							},
							DefaultMode: &mode,
						},
					},
			...
			configMap, _ := kubernetes.GetConfigMap(ctx, k8sClient, controller.RolePermissionsConfigMapName(toCreate), key.Namespace)
