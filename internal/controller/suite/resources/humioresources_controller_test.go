...
const (
	...
	enableScheduledReportEnv   string = "ENABLE_SCHEDULED_REPORT"
	testPDFCAResourceName      string = "pdf-ca-secret"
)
...
var _ = Describe("Humio Resources Controllers", func() {
	...
	Context("Humio Ingest Token", Label("envtest", "dummy", "real"), func() {
		It("should handle ingest token with target secret correctly", func() {
			...
			Eventually(func() *humiographql.IngestTokenDetailsParser {
				humioIngestToken, _ = humioClient.GetIngestToken(ctx, humioHttpClient, fetchedIngestToken)
	...
	Context("Humio Repository and View", Label("envtest", "dummy", "real"), func() {
		It("should handle resources correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				initialRepository, err = humioClient.GetRepository(ctx, humioHttpClient, toCreateRepository)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() repositoryExpectation {
				initialRepository, err := humioClient.GetRepository(ctx, humioHttpClient, fetchedRepository)
				if err != nil {
			...
			Eventually(func() error {
				updatedRepository, err = humioClient.GetRepository(ctx, humioHttpClient, fetchedRepository)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
	...
	Context("Humio Parser", Label("envtest", "dummy", "real"), func() {
		It("HumioParser: Should handle parser correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				initialParser, err = humioClient.GetParser(ctx, humioHttpClient, toCreateParser)
				if err != nil {
					return err
				}
			...
			Eventually(func() error {
				updatedParser, err = humioClient.GetParser(ctx, humioHttpClient, fetchedParser)
	...
	Context("Humio Action", Label("envtest", "dummy", "real"), func() {
		It("should handle email action correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				action, err = humioClient.GetAction(ctx, humioHttpClient, toCreateAction)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() error {
				expectedUpdatedAction, err = humioClient.GetAction(ctx, humioHttpClient, fetchedAction)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() *string {
				updatedAction2, err = humioClient.GetAction(ctx, humioHttpClient, fetchedAction)
				if err != nil {
		...
		It("should handle web hook action correctly", func() {
			...
			Eventually(func() string {
				updatedAction, err = humioClient.GetAction(ctx, humioHttpClient, fetchedAction)
		...
		It("HumioAction: Should deny improperly configured action with missing properties", func() {
			...
			suite.UsingClusterBy(clusterKey.Name, "HumioAction: Confirming creation of invalid action gets rejected")
	...
	Context("Humio Alert", Label("envtest", "dummy", "real"), func() {
		It("should handle alert action correctly", func() {
			...
			Eventually(func() error {
				expectedUpdatedAlert, err = humioClient.GetAlert(ctx, humioHttpClient, fetchedAlert)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() *humiographql.AlertDetails {
				updatedAlert, err := humioClient.GetAlert(ctx, humioHttpClient, fetchedAlert)
				if err != nil {
					return nil
				}
	...
	Context("Humio Filter Alert", Label("envtest", "dummy", "real"), func() {
		It("should handle filter alert action correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				filterAlert, err = humioClient.GetFilterAlert(ctx, humioHttpClient, toCreateFilterAlert)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			Expect(filterAlert).ToNot(BeNil())

			Eventually(func() error {
				return humioClient.ValidateActionsForFilterAlert(ctx, humioHttpClient, toCreateFilterAlert)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() error {
				expectedUpdatedFilterAlert, err = humioClient.GetFilterAlert(ctx, humioHttpClient, fetchedFilterAlert)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() *humiographql.FilterAlertDetails {
				updatedFilterAlert, err := humioClient.GetFilterAlert(ctx, humioHttpClient, fetchedFilterAlert)
				if err != nil {
					return nil
				}
	...
	Context("Humio Aggregate Alert", Label("envtest", "dummy", "real"), func() {
		It("should handle aggregate alert action correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				aggregateAlert, err = humioClient.GetAggregateAlert(ctx, humioHttpClient, toCreateAggregateAlert)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			Expect(aggregateAlert).ToNot(BeNil())

			Eventually(func() error {
				return humioClient.ValidateActionsForAggregateAlert(ctx, humioHttpClient, toCreateAggregateAlert)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() error {
				expectedUpdatedAggregateAlert, err = humioClient.GetAggregateAlert(ctx, humioHttpClient, fetchedAggregateAlert)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() *humiographql.AggregateAlertDetails {
				updatedAggregateAlert, err := humioClient.GetAggregateAlert(ctx, humioHttpClient, fetchedAggregateAlert)
				if err != nil {
					return nil
				}
	...
	Context("Humio Scheduled Search", Label("envtest", "dummy", "real"), func() {
		It("should handle scheduled search action correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				scheduledSearch, err = humioClient.GetScheduledSearch(ctx, humioHttpClient, toCreateScheduledSearch)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			Expect(scheduledSearch).ToNot(BeNil())

			Eventually(func() error {
				return humioClient.ValidateActionsForScheduledSearch(ctx, humioHttpClient, toCreateScheduledSearch)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() error {
				expectedUpdatedScheduledSearch, err = humioClient.GetScheduledSearch(ctx, humioHttpClient, fetchedScheduledSearch)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() *humiographql.ScheduledSearchDetails {
				updatedScheduledSearch, err := humioClient.GetScheduledSearch(ctx, humioHttpClient, fetchedScheduledSearch)
				if err != nil {
					return nil
				}
	...
	Context("Humio User", Label("envtest", "dummy", "real"), func() {
		It("HumioUser: Should handle user correctly", func() {
			...
			humioHttpClient := humioClient.GetHumioHttpClient(sharedCluster.Config(), reconcile.Request{NamespacedName: clusterKey})
			Eventually(func() error {
				initialUser, err = humioClient.GetUser(ctx, humioHttpClient, toCreateUser)
				if err != nil {
					return err
				}
			...
			Eventually(func() *humiographql.UserDetails {
				updatedUser, err := humioClient.GetUser(ctx, humioHttpClient, fetchedUser)
				if err != nil {
					return nil
				}
	...
	Context("HumioPdfRenderService", Label("envtest", "dummy", "real"), func() {
		...
		Context("Basic PDF Render Service Lifecycle", Label("envtest", "dummy", "real"), func() {
			It("should create and manage PDF render service independently", func() {
				ctx := context.Background()
				key := types.NamespacedName{
					Name:      "basic-pdf-render-service",
					Namespace: clusterKey.Namespace,
				}

				// Create an enabler HumioCluster first
				enablerKey := types.NamespacedName{
					Name:      "enabler-cluster-basic",
					Namespace: clusterKey.Namespace,
				}
				...
				}, mediumTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioPdfRenderServiceStateRunning))
			})
		})
		...
		Context("Feature Flag Integration with HumioCluster", Label("envtest", "dummy", "real"), func() {
			It("should scale down when no cluster has ENABLE_SCHEDULED_REPORT=true", func() {
				...
				// Create PDF service without an enabler cluster
				pdfService := &humiov1alpha1.HumioPdfRenderService{
					ObjectMeta: metav1.ObjectMeta{
						Name:      key.Name,
						Namespace: key.Namespace,
		...
		Context("TLS Configuration", Label("envtest", "dummy", "real"), func() {
			It("should configure TLS when enabled", func() {
				...
				// Create CA secret for TLS
				caSecretName := testPDFCAResourceName
				caCert, err := controller.GenerateCACertificate()
				Expect(err).ToNot(HaveOccurred(), "Failed to generate CA certificate")
				...
				// Create TLS certificate secret that the service expects
				tlsSecretName := helpers.PdfRenderServiceTlsSecretName(key.Name)
				...
				// Verify deployment has TLS configuration
				deploymentKey := types.NamespacedName{
					Name:      helpers.PdfRenderServiceChildName(key.Name),
					Namespace: key.Namespace,
				}
				Eventually(func() bool {
					...
					// Check for TLS env var
					hasTLSEnv := false
					for _, container := range deployment.Spec.Template.Spec.Containers {
						for _, env := range container.Env {
							if env.Name == "HUMIO_PDF_RENDER_USE_TLS" && env.Value == trueValue {
								hasTLSEnv = true
							}
						}
					}

					return hasTLSVolume && hasCAVolume && hasTLSEnv
		...
		Context("Update Operations", Label("envtest", "dummy", "real"), func() {
			It("should update deployment when spec changes", func() {
				...
				}, mediumTimeout, suite.TestInterval).Should(Equal(newImage))
				...
				Eventually(func() error {
					...
					updated.Spec.EnvironmentVariables = []corev1.EnvVar{
				...
				Eventually(func() bool {
					...
					return hasLogLevel && hasCustomVar
				}, mediumTimeout, suite.TestInterval).Should(BeTrue())
			})
		})
		...
		Context("HPA Integration", Label("envtest", "dummy", "real"), func() {
			It("should create HPA when autoscaling is enabled", func() {
				...
				// Create PDF service with autoscaling
				minReplicas := int32(2)
				targetCPU := int32(80)
				pdfService := &humiov1alpha1.HumioPdfRenderService{
					ObjectMeta: metav1.ObjectMeta{
				...
				Expect(k8sClient.Get(ctx, hpaKey, &hpa)).Should(Succeed())
		...
		Context("Resource Management", Label("envtest", "dummy", "real"), func() {
			It("should apply resource requests and limits", func() {
				...
				// Create enabler cluster
				enablerKey := types.NamespacedName{
					Name:      "enabler-cluster-resources",
					Namespace: clusterKey.Namespace,
				}
				enablerCluster := suite.ConstructBasicSingleNodeHumioCluster(enablerKey, true)
				enablerCluster.Spec.CommonEnvironmentVariables = []corev1.EnvVar{
				...
				// Create PDF service with resources
				pdfService := &humiov1alpha1.HumioPdfRenderService{
					ObjectMeta: metav1.ObjectMeta{
						Name:      key.Name,
						Namespace: key.Namespace,
				...
				// Verify resources in deployment
				deploymentKey := types.NamespacedName{
					Name:      helpers.PdfRenderServiceChildName(key.Name),
					Namespace: key.Namespace,
				}
				Eventually(func() bool {
					...
					return cpuRequest.String() == "250m" &&
						memRequest.String() == "512Mi" &&
						cpuLimit.String() == "1" &&
						memLimit.String() == "2Gi"
				}, mediumTimeout, suite.TestInterval).Should(BeTrue())
			})
		...
		Context("HumioCluster Integration via DEFAULT_PDF_RENDER_SERVICE_URL", Label("envtest", "dummy", "real"), func() {
			It("should set DEFAULT_PDF_RENDER_SERVICE_URL on HumioCluster pods", func() {
				...
				humioClusterKey := types.NamespacedName{
					Name:      "cluster-with-pdf-url",
					Namespace: clusterKey.Namespace,
				}

				// Create PDF service first
				pdfService := suite.CreatePdfRenderServiceCR(ctx, k8sClient, pdfKey, false)
				defer suite.CleanupPdfRenderServiceCR(ctx, k8sClient, pdfService)

				// Create HumioCluster with PDF integration
				humioCluster := suite.ConstructBasicSingleNodeHumioCluster(humioClusterKey, true)
				humioCluster.Spec.CommonEnvironmentVariables = []corev1.EnvVar{
					{Name: enableScheduledReportEnv, Value: trueValue},
					{Name: "DEFAULT_PDF_RENDER_SERVICE_URL", Value: fmt.Sprintf("http://%s:%d",
						helpers.PdfRenderServiceChildName(pdfKey.Name), controller.DefaultPdfRenderServicePort)},
				}
				Expect(k8sClient.Create(ctx, humioCluster)).Should(Succeed())
				defer suite.CleanupCluster(ctx, k8sClient, humioCluster)

				// Verify PDF service scales up due to cluster
				deploymentKey := types.NamespacedName{
					Name:      helpers.PdfRenderServiceChildName(pdfKey.Name),
					Namespace: pdfKey.Namespace,
				}
				Eventually(func() int32 {
					...
					return *deployment.Spec.Replicas
				}, mediumTimeout, suite.TestInterval).Should(Equal(int32(1)))

				// Verify cluster has PDF URL configured
				Eventually(func() bool {
					cluster := &humiov1alpha1.HumioCluster{}
					if err := k8sClient.Get(ctx, humioClusterKey, cluster); err != nil {
						return false
					}
					for _, env := range cluster.Spec.CommonEnvironmentVariables {
						if env.Name == "DEFAULT_PDF_RENDER_SERVICE_URL" {
							expectedURL := fmt.Sprintf("http://%s:%d",
								helpers.PdfRenderServiceChildName(pdfKey.Name), controller.DefaultPdfRenderServicePort)
							return env.Value == expectedURL
						}
					}
					return false
				}, mediumTimeout, suite.TestInterval).Should(BeTrue())
		...
		Context("Status Conditions", Label("envtest", "dummy", "real"), func() {
			It("should properly set status conditions", func() {
				...
				Eventually(func() bool {
					...
					// Should have at least Available and Progressing conditions
					hasAvailable := false
					hasProgressing := false
					for _, condition := range updated.Status.Conditions {
						if condition.Type == string(humiov1alpha1.HumioPdfRenderServiceAvailable) {
							hasAvailable = true
						}
						if condition.Type == string(humiov1alpha1.HumioPdfRenderServiceProgressing) {
							hasProgressing = true
						}
					}
					return hasAvailable && hasProgressing
				}, mediumTimeout, suite.TestInterval).Should(BeTrue())
			})
		})

		// Test Case 10: Multiple PDF Services
		Context("Multiple PDF Services", Label("envtest", "dummy", "real"), func() {
			It("should support multiple PDF render services in same namespace", func() {
				...
				// Create multiple PDF services
				pdfServices := make([]*humiov1alpha1.HumioPdfRenderService, 3)
				for i := 0; i < 3; i++ {
					key := types.NamespacedName{
						Name:      fmt.Sprintf("pdf-multi-%d", i),
						Namespace: enablerKey.Namespace,
					}
					pdfServices[i] = suite.CreatePdfRenderServiceCR(ctx, k8sClient, key, false)
				}

				// Cleanup
				defer func() {
					for _, pdf := range pdfServices {
						suite.CleanupPdfRenderServiceCR(ctx, k8sClient, pdf)
					}
				}()
				...
				for i := 0; i < 3; i++ {
					deploymentKey := types.NamespacedName{
						Name:      helpers.PdfRenderServiceChildName(fmt.Sprintf("pdf-multi-%d", i)),
						Namespace: enablerKey.Namespace,
					}
					Eventually(func() error {
						deployment := &appsv1.Deployment{}
	...
	Context("HumioSystemPermissionRole", Label("envtest", "dummy", "real"), func() {
		It("Working config: create it, verify it is there, update it, delete it, validate it is gone", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-system-permission-role",
				Namespace: clusterKey.Namespace,
			}
			toCreateSystemPermissionRole := &humiov1alpha1.HumioSystemPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
					Name:      key.Name,
					Namespace: key.Namespace,
				},
				Spec: humiov1alpha1.HumioSystemPermissionRoleSpec{
					ManagedClusterName: clusterKey.Name,
					Name:               "example-system-permission",
					Permissions: []string{
			...
			suite.UsingClusterBy(clusterKey.Name, "Creating the system permission role custom resource")
			...
			Expect(fetchedRoleDetails.SystemPermissions).Should(HaveExactElements([]humiographql.SystemPermission{
				humiographql.SystemPermissionReadhealthcheck,
			}))

			suite.UsingClusterBy(clusterKey.Name, "Add a permission to custom resource using k8sClient, ChangeUsername")
			Eventually(func() error {
				updatedHumioSystemPermissionRole := humiov1alpha1.HumioSystemPermissionRole{}
				err = k8sClient.Get(ctx, key, &updatedHumioSystemPermissionRole)
				if err != nil {
				...
				updatedHumioSystemPermissionRole.Spec.Permissions = append(updatedHumioSystemPermissionRole.Spec.Permissions, string(humiographql.SystemPermissionChangeusername))
				return k8sClient.Update(ctx, &updatedHumioSystemPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() ([]humiographql.SystemPermission, error) {
				...
				return fetchedRoleDetails.SystemPermissions, err
			}, testTimeout, suite.TestInterval).Should(HaveLen(2))
			Expect(fetchedRoleDetails.SystemPermissions).Should(HaveExactElements([]humiographql.SystemPermission{
				humiographql.SystemPermissionChangeusername,
				humiographql.SystemPermissionReadhealthcheck,
			}))
			...
			Eventually(func() error {
				...
				updatedHumioSystemPermissionRole.Spec.Permissions = []string{string(humiographql.SystemPermissionChangeusername)}
				return k8sClient.Update(ctx, &updatedHumioSystemPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			suite.UsingClusterBy(clusterKey.Name, "Delete custom resource using k8sClient")
			Expect(k8sClient.Delete(ctx, toCreateSystemPermissionRole)).To(Succeed())
			Eventually(func() bool {
				err := k8sClient.Get(ctx, key, toCreateSystemPermissionRole)
				return k8serrors.IsNotFound(err)
			}, testTimeout, suite.TestInterval).Should(BeTrue())
		...
		It("Should indicate improperly configured system permission role with unknown permission", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-system-permission-role-unknown-perm",
				Namespace: clusterKey.Namespace,
			}
			toCreateInvalidSystemPermissionRole := &humiov1alpha1.HumioSystemPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
					Name:      key.Name,
					Namespace: key.Namespace,
				},
			...
			suite.UsingClusterBy(clusterKey.Name, "Creating the system permission role with unknown permission")
			Expect(k8sClient.Create(ctx, toCreateInvalidSystemPermissionRole)).Should(Succeed())

			suite.UsingClusterBy(clusterKey.Name, "System permission role should be marked with NotFound")
			Eventually(func() string {
				updatedHumioSystemPermissionRole := humiov1alpha1.HumioSystemPermissionRole{}
				err = k8sClient.Get(ctx, key, &updatedHumioSystemPermissionRole)
				if err != nil {
				...
				return updatedHumioSystemPermissionRole.Status.State
			}, testTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioSystemPermissionRoleStateNotFound))
		})

		It("Should deny improperly configured system permission role with empty list of permissions", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-system-permission-role-empty-list",
				Namespace: clusterKey.Namespace,
			}
			toCreateInvalidSystemPermissionRole := &humiov1alpha1.HumioSystemPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
				...
				Spec: humiov1alpha1.HumioSystemPermissionRoleSpec{
					ManagedClusterName: clusterKey.Name,
					Name:               "example-invalid-system-permission-role",
					Permissions:        nil,
				},
			}

			suite.UsingClusterBy(clusterKey.Name, "Creating the system permission role with nil slice")
			Expect(k8sClient.Create(ctx, toCreateInvalidSystemPermissionRole)).Should(Not(Succeed()))

			toCreateInvalidSystemPermissionRole.Spec.Permissions = []string{}
		...
		It("system permission role gets assigned", func() {
			...
			toCreateSystemPermissionRole := &humiov1alpha1.HumioSystemPermissionRole{
				...
				Spec: humiov1alpha1.HumioSystemPermissionRoleSpec{
					ManagedClusterName: clusterKey.Name,
					Name:               "example-system-permission-assignment",
					Permissions: []string{
						string(humiographql.SystemPermissionReadhealthcheck),
					},
			...
			suite.UsingClusterBy(clusterKey.Name, "Custom resource for system permission group should be marked with Exists")
			Eventually(func() string {
				updatedHumioGroup := humiov1alpha1.HumioGroup{}
				err = k8sClient.Get(ctx, key, &updatedHumioGroup)
				if err != nil {
			...
			suite.UsingClusterBy(clusterKey.Name, "Confirming the system permission role is assigned to the group")
			Eventually(func() error {
				role, err := humioClient.GetSystemPermissionRole(ctx, humioHttpClient, toCreateSystemPermissionRole)
				if err != nil {
					return err
				}
				for _, role := range role.GetGroups() {
					if role.GetDisplayName() == toCreateGroup.Name {
						return nil
					}
				}
				return fmt.Errorf("could not find role assignment to group")
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// Remove system permission role from group
			Eventually(func() error {
				updatedHumioSystemPermissionRole := humiov1alpha1.HumioSystemPermissionRole{}
				err = k8sClient.Get(ctx, key, &updatedHumioSystemPermissionRole)
				if err != nil {
				...
				updatedHumioSystemPermissionRole.Spec.RoleAssignmentGroupNames = []string{}
				return k8sClient.Update(ctx, &updatedHumioSystemPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			suite.UsingClusterBy(clusterKey.Name, "Confirming the system permission role was unassigned from the group")
			Eventually(func() []humiographql.RoleDetailsGroupsGroup {
				role, err := humioClient.GetSystemPermissionRole(ctx, humioHttpClient, toCreateSystemPermissionRole)
				if err != nil {
					return []humiographql.RoleDetailsGroupsGroup{
						{
			...
			suite.UsingClusterBy(clusterKey.Name, "Cleaning up resources")
	...
	Context("HumioOrganizationPermissionRole", Label("envtest", "dummy", "real"), func() {
		...
		It("organization permission role gets assigned", func() {
			...
			toCreateOrganizationPermissionRole := &humiov1alpha1.HumioOrganizationPermissionRole{
				...
				Spec: humiov1alpha1.HumioOrganizationPermissionRoleSpec{
					ManagedClusterName: clusterKey.Name,
					Name:               "example-organization-permission-assignment",
					Permissions: []string{
			...
			suite.UsingClusterBy(clusterKey.Name, "Custom resource for organization permission group should be marked with Exists")
			Eventually(func() string {
				updatedHumioGroup := humiov1alpha1.HumioGroup{}
				err = k8sClient.Get(ctx, key, &updatedHumioGroup)
				if err != nil {
			...
			suite.UsingClusterBy(clusterKey.Name, "Confirming the organization permission role is assigned to the group")
			Eventually(func() error {
				role, err := humioClient.GetOrganizationPermissionRole(ctx, humioHttpClient, toCreateOrganizationPermissionRole)
				if err != nil {
					return err
				}
				for _, role := range role.GetGroups() {
			...
			// Remove organization permission role from group
			Eventually(func() error {
				updatedHumioOrganizationPermissionRole := humiov1alpha1.HumioOrganizationPermissionRole{}
				err = k8sClient.Get(ctx, key, &updatedHumioOrganizationPermissionRole)
				if err != nil {
				...
				updatedHumioOrganizationPermissionRole.Spec.RoleAssignmentGroupNames = []string{}
				return k8sClient.Update(ctx, &updatedHumioOrganizationPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			suite.UsingClusterBy(clusterKey.Name, "Confirming the organization permission role was unassigned from the group")
			Eventually(func() []humiographql.RoleDetailsGroupsGroup {
				role, err := humioClient.GetOrganizationPermissionRole(ctx, humioHttpClient, toCreateOrganizationPermissionRole)
				if err != nil {
					return []humiographql.RoleDetailsGroupsGroup{
	...
	Context("HumioViewPermissionRole", Label("envtest", "dummy", "real"), func() {
		It("Working config: create it, verify it is there, update it, delete it, validate it is gone", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-view-permission-role",
				Namespace: clusterKey.Namespace,
			}
			toCreateViewPermissionRole := &humiov1alpha1.HumioViewPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
					Name:      key.Name,
					Namespace: key.Namespace,
				},
			...
			suite.UsingClusterBy(clusterKey.Name, "Creating the view permission role custom resource")
			...
			Expect(fetchedRoleDetails.ViewPermissions).Should(HaveExactElements([]humiographql.Permission{
				humiographql.PermissionReadaccess,
			}))
			...
			Eventually(func() error {
				...
				updatedHumioViewPermissionRole.Spec.Permissions = append(updatedHumioViewPermissionRole.Spec.Permissions, string(humiographql.PermissionChangeretention))
				return k8sClient.Update(ctx, &updatedHumioViewPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			Eventually(func() ([]humiographql.Permission, error) {
				...
				Expect(fetchedRoleDetails).ToNot(BeNil())
				return fetchedRoleDetails.ViewPermissions, err
			}, testTimeout, suite.TestInterval).Should(HaveLen(2))
			Expect(fetchedRoleDetails.ViewPermissions).Should(HaveExactElements([]humiographql.Permission{
				humiographql.PermissionChangeretention,
				humiographql.PermissionReadaccess,
			}))
			...
			Eventually(func() error {
				...
				updatedHumioViewPermissionRole.Spec.Permissions = []string{string(humiographql.PermissionChangeretention)}
				return k8sClient.Update(ctx, &updatedHumioViewPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			suite.UsingClusterBy(clusterKey.Name, "Delete custom resource using k8sClient")
			Expect(k8sClient.Delete(ctx, toCreateViewPermissionRole)).To(Succeed())
			Eventually(func() bool {
				err := k8sClient.Get(ctx, key, toCreateViewPermissionRole)
				return k8serrors.IsNotFound(err)
			}, testTimeout, suite.TestInterval).Should(BeTrue())
		...
		It("Should indicate improperly configured view permission role with unknown permission", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-view-permission-role-unknown-perm",
				Namespace: clusterKey.Namespace,
			}
			toCreateInvalidViewPermissionRole := &humiov1alpha1.HumioViewPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
					Name:      key.Name,
					Namespace: key.Namespace,
				},
			...
			suite.UsingClusterBy(clusterKey.Name, "Creating the view permission role with unknown permission")
			Expect(k8sClient.Create(ctx, toCreateInvalidViewPermissionRole)).Should(Succeed())
			...
			Eventually(func() string {
				...
				return updatedHumioViewPermissionRole.Status.State
			}, testTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioViewPermissionRoleStateNotFound))
		})

		It("Should deny improperly configured view permission role with empty list of permissions", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humio-view-permission-role-empty-list",
				Namespace: clusterKey.Namespace,
			}
			toCreateInvalidViewPermissionRole := &humiov1alpha1.HumioViewPermissionRole{
				ObjectMeta: metav1.ObjectMeta{
			...
			suite.UsingClusterBy(clusterKey.Name, "Creating the view permission role with nil slice")
			Expect(k8sClient.Create(ctx, toCreateInvalidViewPermissionRole)).Should(Not(Succeed()))

			toCreateInvalidViewPermissionRole.Spec.Permissions = []string{}
		...
		It("view permission role gets assigned", func() {
			...
			toCreateViewPermissionRole := &humiov1alpha1.HumioViewPermissionRole{
				...
				Spec: humiov1alpha1.HumioViewPermissionRoleSpec{
					...
					RoleAssignments: []humiov1alpha1.HumioViewPermissionRoleAssignment{
						{
							RepoOrViewName: toCreateRepository.Spec.Name,
			...
			suite.UsingClusterBy(clusterKey.Name, "Custom resource for repository should be marked with Exists")
			Eventually(func() string {
				updatedHumioRepository := humiov1alpha1.HumioRepository{}
				err = k8sClient.Get(ctx, key, &updatedHumioRepository)
				if err != nil {
					return err.Error()
				}
				return updatedHumioRepository.Status.State
			}, testTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioRepositoryStateExists))
			suite.UsingClusterBy(clusterKey.Name, "Custom resource for group should be marked with Exists")
			Eventually(func() string {
				updatedHumioGroup := humiov1alpha1.HumioGroup{}
			...
			suite.UsingClusterBy(clusterKey.Name, "Confirming the repository does exist in LogScale after custom resource indicates that it does")
			Eventually(func() error {
				_, err = humioClient.GetRepository(ctx, humioHttpClient, toCreateRepository)
				return err
			}, testTimeout, suite.TestInterval).Should(Succeed())
			suite.UsingClusterBy(clusterKey.Name, "Confirming the group does exist in LogScale after custom resource indicates that it does")
			Eventually(func() error {
			...
			suite.UsingClusterBy(clusterKey.Name, "Confirming the view permission role is assigned to the group")
			Eventually(func() error {
				role, err := humioClient.GetViewPermissionRole(ctx, humioHttpClient, toCreateViewPermissionRole)
				if err != nil {
					return err
				}
				for _, role := range role.GetGroups() {
					if role.GetDisplayName() == toCreateGroup.Name &&
						len(role.GetRoles()) == len(toCreateViewPermissionRole.Spec.RoleAssignments) {
						return nil
					}
				}
				return fmt.Errorf("did not find expected role assignment")
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// Remove view permission role from group
			Eventually(func() error {
				updatedHumioViewPermissionRole := humiov1alpha1.HumioViewPermissionRole{}
				err = k8sClient.Get(ctx, key, &updatedHumioViewPermissionRole)
				if err != nil {
				...
				updatedHumioViewPermissionRole.Spec.RoleAssignments = []humiov1alpha1.HumioViewPermissionRoleAssignment{}
				return k8sClient.Update(ctx, &updatedHumioViewPermissionRole)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			suite.UsingClusterBy(clusterKey.Name, "Confirming the view permission role was unassigned from the group")
			Eventually(func() []humiographql.RoleDetailsGroupsGroup {
				role, err := humioClient.GetViewPermissionRole(ctx, humioHttpClient, toCreateViewPermissionRole)
				if err != nil {
					return []humiographql.RoleDetailsGroupsGroup{
	...
	// PDF Render Service Tests - Triggered by HumioCluster Environment Variable
	// These tests verify that HumioPdfRenderService operates independently
	// but is only deployed when at least one HumioCluster has ENABLE_SCHEDULED_REPORT=true.
	// HumioClusters interact with PDF services via DEFAULT_PDF_RENDER_SERVICE_URL
	// environment variable set on LogScale pods.
	const (
		shortTimeout  = time.Second * 10
		mediumTimeout = time.Second * 30
		longTimeout   = time.Second * 60
	...
	FContext("PDF Render Service Triggered by HumioCluster ENABLE_SCHEDULED_REPORT", Label("envtest", "dummy", "real"), func() {
		It("should be enabled when at least one HumioCluster has ENABLE_SCHEDULED_REPORT=true", func() {
			...
			hprsKey := types.NamespacedName{
				Name:      "humio-pdf-render-service-triggered",
				Namespace: clusterKey.Namespace,
			}
			depKey := types.NamespacedName{
				Name:      helpers.PdfRenderServiceChildName(hprsKey.Name),
				Namespace: hprsKey.Namespace,
			}

			// Clean up any existing resources first
			suite.CleanupPdfRenderServiceResources(ctx, k8sClient, hprsKey)

			// Create a HumioCluster with ENABLE_SCHEDULED_REPORT=true
			suite.UsingClusterBy(clusterKey.Name, "Creating HumioCluster with ENABLE_SCHEDULED_REPORT=true")
			tempClusterKey := types.NamespacedName{
				Name:      "temp-cluster-for-pdf",
				Namespace: clusterKey.Namespace,
			}
			tempHumioCluster := suite.ConstructBasicSingleNodeHumioCluster(tempClusterKey, true)
			tempHumioCluster.Spec.CommonEnvironmentVariables = append(
				tempHumioCluster.Spec.CommonEnvironmentVariables,
				corev1.EnvVar{
					Name:  enableScheduledReportEnv,
					Value: trueValue,
				},
			)
			Expect(k8sClient.Create(ctx, tempHumioCluster)).To(Succeed())
			defer suite.CleanupCluster(ctx, k8sClient, tempHumioCluster)

			suite.UsingClusterBy(clusterKey.Name, "Creating HumioPdfRenderService CR - should now be enabled")
			...
			suite.UsingClusterBy(clusterKey.Name, "Verifying PDF service reaches Running state independently")
			Eventually(func() string {
				var updatedHprs humiov1alpha1.HumioPdfRenderService
				_ = k8sClient.Get(ctx, hprsKey, &updatedHprs)
				return updatedHprs.Status.State
			}, longTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioPdfRenderServiceStateRunning))
			...
			suite.UsingClusterBy(clusterKey.Name, "Cleaning up remaining PDF service resources")
			suite.CleanupPdfRenderServiceResources(ctx, k8sClient, hprsKey)
		})
	})
	...
	FContext("PDF Render Service Creation When Enabled", Label("envtest", "dummy", "real"), func() {
		...
		It("should create Deployment and Service when HumioCluster enables scheduled reports", func() {
			...
			// Create a HumioCluster with ENABLE_SCHEDULED_REPORT=true first
			enablerClusterKey := types.NamespacedName{
				Name:      "enabler-cluster-for-pdf",
				Namespace: clusterKey.Namespace,
			}
			...
			suite.CreateAndBootstrapCluster(ctx, k8sClient, humioClient, enablerHumioCluster, true, humiov1alpha1.HumioClusterStateRunning, testTimeout*2)
			...
			suite.UsingClusterBy(clusterKey.Name, "Verifying PDF service reaches Running state")
			Eventually(func() string {
				var updatedHprs humiov1alpha1.HumioPdfRenderService
				...
				return updatedHprs.Status.State
			}, testTimeout*2, suite.TestInterval).Should(Equal(humiov1alpha1.HumioPdfRenderServiceStateRunning))
			...
			expectedName := helpers.PdfRenderServiceChildName(pdfKey.Name)
			...
			suite.UsingClusterBy(clusterKey.Name, "Cleaning up HumioPdfRenderService CR")
			suite.CleanupPdfRenderServiceCR(ctx, k8sClient, hprs)
		})
	})
	...
	Context("PDF Render Service Update", Label("envtest", "dummy", "real"), func() {
		...
		It("should update the Deployment when the HumioPdfRenderService is updated", func() {
			...
			suite.UsingClusterBy(clusterKey.Name, "Waiting for PDF service to detect enabler cluster and scale up")
			Eventually(func() int32 {
				deployment := &appsv1.Deployment{}
				if err := k8sClient.Get(ctx, deploymentKey, deployment); err != nil {
					return 0
				...
				return *deployment.Spec.Replicas
			}, testTimeout*2, suite.TestInterval).Should(BeNumerically(">", 0), "PDF service deployment should scale up when enabler cluster is detected")
			...
			Eventually(func() string {
				...
				return updated.Status.State
			}, testTimeout, suite.TestInterval).Should(Equal(humiov1alpha1.HumioPdfRenderServiceStateRunning))
			...
			Eventually(func() error {
				...
				// Explicitly disable autoscaling to ensure manual replica scaling works
				freshHprs.Spec.Autoscaling = &humiov1alpha1.HumioPdfRenderServiceAutoscalingSpec{
			...
			suite.UsingClusterBy(clusterKey.Name, "Cleaning up HumioPdfRenderService CR")
			suite.CleanupPdfRenderServiceCR(ctx, k8sClient, freshHprs)

			suite.UsingClusterBy(clusterKey.Name, "Cleaning up enabler cluster")
			suite.CleanupCluster(ctx, k8sClient, enablerCluster)
	...
	Context("PDF Render Service Resources and Probes", Label("envtest", "dummy", "real"), func() {
		It("should correctly set up resources and probes when specified", func() {
			...
			// Create the HumioPdfRenderService with resources & probes
			hprs := &humiov1alpha1.HumioPdfRenderService{
			...
			// Assert resources & probes
			dep := &appsv1.Deployment{}
			Eventually(func() error { return k8sClient.Get(ctx, deploymentKey, dep) },
				longTimeout, suite.TestInterval).Should(Succeed())

			Expect(dep.Spec.Template.Spec.Containers).NotTo(BeEmpty())
			c := dep.Spec.Template.Spec.Containers[0]

			Expect(c.Resources.Limits.Cpu().String()).To(Equal("500m"))
			Expect(c.Resources.Limits.Memory().String()).To(Equal("512Mi"))
			Expect(c.Resources.Requests.Cpu().String()).To(Equal("100m"))
			Expect(c.Resources.Requests.Memory().String()).To(Equal("128Mi"))

			Expect(c.LivenessProbe).NotTo(BeNil())
			Expect(c.LivenessProbe.HTTPGet.Path).To(Equal("/health"))
			Expect(c.LivenessProbe.InitialDelaySeconds).To(Equal(int32(30)))
			Expect(c.LivenessProbe.TimeoutSeconds).To(Equal(int32(60)))

			Expect(c.ReadinessProbe).NotTo(BeNil())
			Expect(c.ReadinessProbe.HTTPGet.Path).To(Equal("/ready"))
			Expect(c.ReadinessProbe.InitialDelaySeconds).To(Equal(int32(30)))
			Expect(c.ReadinessProbe.TimeoutSeconds).To(Equal(int32(60)))
	...
	Context("PDF Render Service Environment Variables", Label("envtest", "dummy", "real"), func() {
		It("should correctly configure environment variables (create and update)", func() {
			...
			// Create the CR with custom env vars
			hprs := &humiov1alpha1.HumioPdfRenderService{
				ObjectMeta: metav1.ObjectMeta{Name: key.Name, Namespace: key.Namespace},
				Spec: humiov1alpha1.HumioPdfRenderServiceSpec{
					Replicas: 1,
	...
	Context("PDF Render Service with HumioCluster Environment Variable Integration", Label("envtest", "dummy", "real"), func() {
		It("Should demonstrate HumioCluster interaction with PDF service via DEFAULT_PDF_RENDER_SERVICE_URL", func() {
			...
			pdfKey := types.NamespacedName{Name: "auto-deployed-pdf-service", Namespace: clusterKey.Namespace}

			// Start from a clean slate
			tempCluster := &humiov1alpha1.HumioCluster{}
			if err := k8sClient.Get(ctx, clusterKey, tempCluster); err == nil {
			...
			depKey := types.NamespacedName{Name: helpers.PdfRenderServiceChildName(pdfKey.Name), Namespace: pdfKey.Namespace}

			suite.CleanupPdfRenderServiceResources(ctx, k8sClient, pdfKey)

			// Create HumioCluster with ENABLE_SCHEDULED_REPORT and DEFAULT_PDF_RENDER_SERVICE_URL for API interaction FIRST
			// This enables the PDF service to operate properly
			humioCluster.Spec.CommonEnvironmentVariables = append(
				humioCluster.Spec.CommonEnvironmentVariables,
				corev1.EnvVar{
					Name:  enableScheduledReportEnv,
					Value: trueValue,
				},
			...
			suite.CreateAndBootstrapCluster(ctx, k8sClient, humioClient, humioCluster, true, humiov1alpha1.HumioClusterStateRunning, testTimeout*2)
			defer suite.CleanupCluster(ctx, k8sClient, humioCluster)

			// Now create the PDF service (after enabler cluster exists)
			pdfCR := suite.CreatePdfRenderServiceCR(ctx, k8sClient, pdfKey, false)

			// ------------------------------------------------------------------
			// First image change – use a fresh object to avoid RV-conflict (409)
			// ------------------------------------------------------------------
			Eventually(func() error {
				current := &humiov1alpha1.HumioPdfRenderService{}
				if err := k8sClient.Get(ctx, pdfKey, current); err != nil {
					return err
				}
				current.Spec.Image = customPdfImage
				return k8sClient.Update(ctx, current)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// refresh the local copy so subsequent helpers have the right
			// generation / resourceVersion
			Expect(k8sClient.Get(ctx, pdfKey, pdfCR)).To(Succeed())

			suite.WaitForObservedGeneration(ctx, k8sClient, pdfCR, longTimeout, suite.TestInterval)
			suite.EnsurePdfRenderDeploymentReady(ctx, k8sClient, depKey)

			Eventually(func() string { // assert custom image
				d := appsv1.Deployment{}
				_ = k8sClient.Get(ctx, depKey, &d)
				if len(d.Spec.Template.Spec.Containers) == 0 {
					return ""
				}
				return d.Spec.Template.Spec.Containers[0].Image
			}, testTimeout, suite.TestInterval).Should(Equal(customPdfImage))

			//  Update image, then reset to default
			for _, img := range []string{"updated/pdf-render-service:2.0.0", versions.DefaultPDFRenderServiceImage()} {
				Eventually(func() error {
					cr := humiov1alpha1.HumioPdfRenderService{}
					if err := k8sClient.Get(ctx, pdfKey, &cr); err != nil {
						return err
					}
					cr.Spec.Image = img
				...
				Eventually(func() string {
					...
					return d.Spec.Template.Spec.Containers[0].Image
				}, testTimeout, suite.TestInterval).Should(Equal(img))
			}

			// Disable ENABLE_SCHEDULED_REPORT then remove CR
			Eventually(func() error {
				hc := humiov1alpha1.HumioCluster{}
				if err := k8sClient.Get(ctx, clusterKey, &hc); err != nil {
					return err
				}
				hc.Spec.CommonEnvironmentVariables = slices.DeleteFunc(hc.Spec.CommonEnvironmentVariables, func(ev corev1.EnvVar) bool {
					return ev.Name == enableScheduledReportEnv || ev.Name == "DEFAULT_PDF_RENDER_SERVICE_URL"
				})
				return k8sClient.Update(ctx, &hc)
			}, testTimeout, suite.TestInterval).Should(Succeed())
	...
	Context("PDF Render Service HPA (Horizontal Pod Autoscaling)", Label("envtest", "dummy", "real"), func() {
		...
		It("should support both CPU and memory metrics", func() {
			...
			// Create a HumioCluster with ENABLE_SCHEDULED_REPORT=true first
			enablerClusterKey := types.NamespacedName{
				Name:      "enabler-cluster-multi-metrics",
				Namespace: clusterKey.Namespace,
			}
			enablerHumioCluster := suite.ConstructBasicSingleNodeHumioCluster(enablerClusterKey, true)
			enablerHumioCluster.Spec.CommonEnvironmentVariables = append(
		...
		It("should delete HPA when autoscaling is disabled after being enabled", func() {
			...
			// Create a HumioCluster with ENABLE_SCHEDULED_REPORT=true first
			enablerClusterKey := types.NamespacedName{
				Name:      "enabler-cluster-toggle-hpa",
				Namespace: clusterKey.Namespace,
			}
			enablerHumioCluster := suite.ConstructBasicSingleNodeHumioCluster(enablerClusterKey, true)
			enablerHumioCluster.Spec.CommonEnvironmentVariables = append(
			...
			Eventually(func() error {
				...
				updatedHprs.Spec.Autoscaling.Enabled = helpers.BoolPtr(false)
				return k8sClient.Update(ctx, &updatedHprs)
			}, shortTimeout, suite.TestInterval).Should(Succeed())
		...
		It("should use default CPU metric when no metrics are specified", func() {
			...
			// Create the HumioPdfRenderService FIRST so it exists when the enabler cluster triggers the watch
			suite.UsingClusterBy(clusterKey.Name, "Creating HumioPdfRenderService with autoscaling but no metrics specified")
			hprs := suite.CreatePdfRenderServiceCR(ctx, k8sClient, key, false)
			defer suite.CleanupPdfRenderServiceResources(ctx, k8sClient, key)

			// Create a HumioCluster with ENABLE_SCHEDULED_REPORT=true AFTER the PDF service
			// This ensures the watch mechanism will trigger reconciliation when the cluster is created
			enablerClusterKey := types.NamespacedName{
				Name:      "enabler-cluster-default-metrics",
				Namespace: clusterKey.Namespace,
			}
			enablerHumioCluster := suite.ConstructBasicSingleNodeHumioCluster(enablerClusterKey, true)
			enablerHumioCluster.Spec.CommonEnvironmentVariables = append(
			...
			// Wait for the enabler cluster to be fully reconciled
			suite.UsingClusterBy(enablerClusterKey.Name, "Waiting for enabler cluster to be fully reconciled")
			Eventually(func() bool {
				var cluster humiov1alpha1.HumioCluster
				err := k8sClient.Get(ctx, enablerClusterKey, &cluster)
				return err == nil && cluster.Status.State == humiov1alpha1.HumioClusterStateRunning
			}, testTimeout, suite.TestInterval).Should(BeTrue())
			...
			suite.UsingClusterBy(enablerClusterKey.Name, "Waiting for observedGeneration to catch up")
			suite.WaitForObservedGeneration(ctx, k8sClient, hprs, longTimeout, suite.TestInterval)

			suite.UsingClusterBy(enablerClusterKey.Name, "Ensuring Deployment is ready before HPA")
			suite.EnsurePdfRenderDeploymentReady(ctx, k8sClient, depKey)

			suite.UsingClusterBy(enablerClusterKey.Name, "Verifying HPA uses default 80% CPU metric")
			Eventually(func() bool {
				hpa := &autoscalingv2.HorizontalPodAutoscaler{}
				err := k8sClient.Get(ctx, hpaKey, hpa)
				if err != nil {
			...
			suite.UsingClusterBy(enablerClusterKey.Name, "Cleaning up HumioPdfRenderService CR")
			suite.CleanupPdfRenderServiceCR(ctx, k8sClient, hprs)
		})
	...
	Context("PDF Render Service Reconcile Loop", Label("envtest", "dummy", "real"), func() {
		It("should not trigger update when ImagePullPolicy is explicitly set as default", func() {
			ctx := context.Background()
			key := types.NamespacedName{
				Name:      "humiopdf-pull-policy-test",
				Namespace: clusterKey.Namespace,
			}
			deploymentKey := types.NamespacedName{
				Name:      helpers.PdfRenderServiceChildName(key.Name),
			...
			// Create a HumioCluster to enable PDF service
			enablerClusterKey := types.NamespacedName{
				Name:      "enabler-cluster-for-pull-policy",
				Namespace: clusterKey.Namespace,
			}
			enablerHumioCluster := suite.ConstructBasicSingleNodeHumioCluster(enablerClusterKey, true)
			enablerHumioCluster.Spec.CommonEnvironmentVariables = append(
			...
			By("Creating HumioPdfRenderService with explicit default ImagePullPolicy")
			hprs := &humiov1alpha1.HumioPdfRenderService{
				ObjectMeta: metav1.ObjectMeta{Name: key.Name, Namespace: key.Namespace},
				Spec: humiov1alpha1.HumioPdfRenderServiceSpec{
					Image:           "image/name:1.2.3", // non-latest
					ImagePullPolicy: corev1.PullIfNotPresent,
					Replicas:        1,
				},
			}
			Expect(k8sClient.Create(ctx, hprs)).To(Succeed())
			defer suite.CleanupPdfRenderServiceCR(ctx, k8sClient, hprs)

			By("waiting for deployment to be ready")
			suite.WaitForObservedGeneration(ctx, k8sClient, hprs, testTimeout, suite.TestInterval)
			suite.EnsurePdfRenderDeploymentReady(ctx, k8sClient, deploymentKey)
			...
			initialGeneration := deployment.Status.ObservedGeneration

			// Consistently check that the generation does not increase
			By("Consistently checking that deployment generation does not change")
			Consistently(func() int64 {
				deploymentAfter := &appsv1.Deployment{}
				_ = k8sClient.Get(ctx, deploymentKey, deploymentAfter)
				return deploymentAfter.Status.ObservedGeneration
			}, 5*time.Second, 1*time.Second).Should(Equal(initialGeneration))
		})
	})
})
