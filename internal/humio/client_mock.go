...
type ClientMock struct {
	...
	ViewToken              map[resourceKey]humiographql.TokenDetailsViewPermissionsToken
}
...
func NewMockClient() *MockClientConfig {
	mockClientConfig := &MockClientConfig{
		apiClient: &ClientMock{
			...
			ViewToken:              make(map[resourceKey]humiographql.TokenDetailsViewPermissionsToken),
		},
	}
...
func (h *MockClientConfig) ClearHumioClientConnections(repoNameToKeep string) {
	...
	h.apiClient.ViewToken = make(map[resourceKey]humiographql.TokenDetailsViewPermissionsToken)
}
...
func (h *MockClientConfig) CreateViewToken(ctx context.Context, client *humioapi.Client, viewToken *humiov1alpha1.HumioViewToken, ipFilter string, views []string, permissions []humiographql.Permission) (string, string, error) {
	...
	// views
	localViews := make([]humiographql.TokenDetailsViewsSearchDomain, 0, len(views))
	for _, viewName := range views {
		view := &humiographql.TokenDetailsViewsView{
			Typename: helpers.StringPtr("View"),
			Id:       viewName,
			Name:     viewName,
		}
...
func (h *MockClientConfig) GetViewToken(ctx context.Context, client *humioapi.Client, viewToken *humiov1alpha1.HumioViewToken) (*humiographql.TokenDetailsViewPermissionsToken, error) {
	humioClientMu.Lock()
	defer humioClientMu.Unlock()
...
func (h *MockClientConfig) UpdateTokenSecurityPolicy(ctx context.Context, client *humioapi.Client, tp humiographql.TokenSecurityPolicies) error {
	return nil
}

func (h *MockClientConfig) NewTokenSecurityPolicies() humiographql.TokenSecurityPolicies {
	return humiographql.TokenSecurityPolicies{
		PersonalUserTokensEnabled:                          true,
		ViewPermissionTokensEnabled:                        true,
		OrganizationPermissionTokensEnabled:                true,
		SystemPermissionTokensEnabled:                      true,
		ViewPermissionTokensAllowPermissionUpdates:         true,
		OrganizationPermissionTokensAllowPermissionUpdates: true,
		SystemPermissionTokensAllowPermissionUpdates:       true,
	}
}
