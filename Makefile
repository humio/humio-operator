...
#	go generate ./... # NOTE: This line was custom added for the humio-operator project.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name humio-operator-builder
	$(CONTAINER_TOOL) buildx use humio-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm humio-operator-builder
	rm Dockerfile.cross
...
##@ The majority of the custom additions to this makefile for the humio-operator projects is below this line
...
test: manifests generate fmt vet setup-envtest ginkgo ## Run tests.
	...
	$(GINKGO) run --focus "Humio ViewToken" --label-filter=envtest -vv --no-color --procs=3 -output-dir=${PWD} -keep-separate-reports -race --junit-report=test-results-junit.xml --randomize-suites --randomize-all -timeout 10m ./internal/controller/suite/resources
