...
test: manifests generate fmt vet setup-envtest ginkgo ## Run tests.
	...
	$(GINKGO) run --focus "ViewToken" --label-filter=envtest -vv --no-color --procs=3 -output-dir=${PWD} -keep-separate-reports -race --junit-report=test-results-junit.xml --randomize-suites --randomize-all -timeout 10m ./internal/controller/suite/resources
