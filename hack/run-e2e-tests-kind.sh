...
USE_CERTMANAGER=true TEST_USE_EXISTING_CLUSTER=true $ginkgo -timeout 90m -nodes=$ginkgo_nodes -skipPackage helpers -race -v ./... -covermode=count -coverprofile cover.out -progress | tee /proc/1/fd/1
