...
USE_CERTMANAGER=true TEST_USE_EXISTING_CLUSTER=true $ginkgo -slow-spec-threshold=5s -timeout 90m -nodes=$ginkgo_nodes --skip-package helpers -race -v ./testbindir/* -covermode=count -coverprofile cover.out -progress | tee /proc/1/fd/1
TEST_EXIT_CODE=$?
