...
const (
	// HumioViewTokenUnknown is the Unknown state of the ingest token
	HumioViewTokenUnknown = "Unknown"
	// HumioViewTokenExists is the Exists state of the ingest token
	HumioViewTokenExists = "Exists"
	// HumioViewTokenNotFound is the NotFound state of the ingest token
...
type HumioViewTokenSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="size(self) <= 63",message="tokenSecretLabels cannot exceed 63 labels"
	...
	// +kubebuilder:validation:XValidation:rule="self.all(key, size(key) <= 63 && size(key) > 0)",message="tokenSecretAnnotations keys must be 1-63 characters"
	// +kubebuilder:validation:XValidation:rule="self.all(key, size(self[key]) <= 63 && size(self[key]) > 0)",message="tokenSecretAnnotations values must be 1-63 characters"
	// +kubebuilder:validation:XValidation:rule="self.map(key, size(key) + size(self[key])).sum() <= 262144",message="tokenSecretAnnotations total size cannot exceed 256KB"
	// +kubebuilder:validation:Optional
	TokenSecretAnnotations map[string]string `json:"tokenSecretAnnotations,omitempty"`
	// TODO AssetPermissionAssignments
}
