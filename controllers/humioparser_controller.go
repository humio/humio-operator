...
func (r *HumioParserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.Log.Info("Reconciling HumioParser", logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	r.Log.Info("Checking if parser is marked to be deleted", logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	if isHumioParserMarkedToBeDeleted {
		r.Log.Info("Parser marked to be deleted", logFieldFunctionName, helpers.GetCurrentFuncName())
		if helpers.ContainsElement(hp.GetFinalizers(), humioFinalizer) {
			// Run finalization logic for humioFinalizer. If the
			// finalization logic fails, don't remove the finalizer so
			// that we can retry during the next reconciliation.
			r.Log.Info("Parser contains finalizer so run finalizer method", logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	if !helpers.ContainsElement(hp.GetFinalizers(), humioFinalizer) {
		r.Log.Info("Finalizer not present, adding finalizer to parser", logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	// Get current parser
	r.Log.Info("get current parser", logFieldFunctionName, helpers.GetCurrentFuncName())
	curParser, err := r.HumioClient.GetParser(hp)
	if err != nil {
		r.Log.Error(err, "could not check if parser exists", "Parser.Name", hp.Spec.RepositoryName, logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	if reflect.DeepEqual(emptyParser, *curParser) {
		r.Log.Info("parser doesn't exist. Now adding parser", logFieldFunctionName, helpers.GetCurrentFuncName())
		// create parser
		_, err := r.HumioClient.AddParser(hp)
		if err != nil {
			r.Log.Error(err, "could not create parser", logFieldFunctionName, helpers.GetCurrentFuncName())
			return reconcile.Result{}, fmt.Errorf("could not create parser: %s", err)
		}
		r.Log.Info("created parser", logFieldFunctionName, helpers.GetCurrentFuncName())
	...
	if (curParser.Script != hp.Spec.ParserScript) || !reflect.DeepEqual(curParser.TagFields, hp.Spec.TagFields) || !reflect.DeepEqual(curParser.Tests, hp.Spec.TestData) {
		r.Log.Info("parser information differs, triggering update", logFieldFunctionName, helpers.GetCurrentFuncName())
		_, err = r.HumioClient.UpdateParser(hp)
		if err != nil {
			r.Log.Error(err, "could not update parser", logFieldFunctionName, helpers.GetCurrentFuncName())
...
func (r *HumioParserReconciler) addFinalizer(ctx context.Context, hp *humiov1alpha1.HumioParser) error {
	r.Log.Info("Adding Finalizer for the HumioParser", logFieldFunctionName, helpers.GetCurrentFuncName())
	hp.SetFinalizers(append(hp.GetFinalizers(), humioFinalizer))

	// Update CR
	err := r.Update(ctx, hp)
	if err != nil {
		r.Log.Error(err, "Failed to update HumioParser with finalizer", logFieldFunctionName, helpers.GetCurrentFuncName())
		return err
	}
	return nil
}
...
func (r *HumioParserReconciler) setState(ctx context.Context, state string, hp *humiov1alpha1.HumioParser) error {
	...
	r.Log.Info(fmt.Sprintf("setting parser state to %s", state), logFieldFunctionName, helpers.GetCurrentFuncName())
