...
var _ = Describe("HumioCluster Controller", func() {
	...
	Context("Humio Cluster Unsupported Version", Label("envtest", "dummy", "real"), func() {
		It("Creating cluster with unsupported version", func() {
			...
			}, testTimeout, suite.TestInterval).Should(Equal(fmt.Sprintf("Humio version must be at least %s: unsupported Humio version: %s", controllers.HumioVersionMinimumSupported, strings.Split(strings.Split(versions.OldUnsupportedHumioVersion(), ":")[1], "-")[0])))
		})
	})
	...
	Context("Humio Cluster Update Failed Pods", Label("envtest", "dummy", "real"), func() {
		It("Update should correctly replace pods that are in a failed state", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 3, 1)
	...
	Context("Humio Cluster Update Image Rolling Restart", Label("envtest", "dummy", "real"), func() {
		It("Update should correctly replace pods to use new image in a rolling fashion", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 2, 1)
	...
	Context("Humio Cluster Update Image Multi Node Pool", Label("envtest", "dummy", "real"), func() {
		It("Update should correctly replace pods to use new image in multiple node pools", func() {
			...
			nonUpdatedClusterPods, _ := kubernetes.ListPods(ctx, k8sClient, updatedHumioCluster.Namespace, controllers.NewHumioNodeManagerFromHumioNodePool(&updatedHumioCluster, &updatedHumioCluster.Spec.NodePools[0]).GetPodLabels())
			...
			ensurePodsSimultaneousRestart(ctx, controllers.NewHumioNodeManagerFromHumioNodePool(&updatedHumioCluster, &updatedHumioCluster.Spec.NodePools[0]), 2)
			...
			updatedHumioCluster = humiov1alpha1.HumioCluster{}
			Expect(k8sClient.Get(ctx, key, &updatedHumioCluster)).Should(Succeed())
			Expect(controllers.NewHumioNodeManagerFromHumioNodePool(&updatedHumioCluster, &updatedHumioCluster.Spec.NodePools[0]).GetDesiredPodRevision()).To(BeEquivalentTo(2))

			updatedClusterPods, _ = kubernetes.ListPods(ctx, k8sClient, updatedHumioCluster.Namespace, controllers.NewHumioNodeManagerFromHumioNodePool(&updatedHumioCluster, &updatedHumioCluster.Spec.NodePools[0]).GetPodLabels())
	...
	Context("Humio Cluster Update Using Wrong Image", Label("envtest", "dummy", "real"), func() {
		It("Update should correctly replace pods after using wrong image", func() {
			...
			updatedHumioCluster = humiov1alpha1.HumioCluster{}
			Expect(k8sClient.Get(ctx, key, &updatedHumioCluster)).Should(Succeed())
			Expect(controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster).GetDesiredPodRevision()).To(BeEquivalentTo(3))
		...
		It("Update should correctly replace pods to use new bootstrap token", func() {
			...
			Eventually(func() string {
				...
				if len(clusterPods) > 0 {
					return clusterPods[0].Annotations[controllers.BootstrapTokenHashAnnotation]
				}
				return ""
			}, testTimeout, suite.TestInterval).Should(Not(Equal("")))

			clusterPods, _ := kubernetes.ListPods(ctx, k8sClient, key.Namespace, controllers.NewHumioNodeManagerFromHumioCluster(toCreate).GetPodLabels())
			bootstrapTokenHashValue := clusterPods[0].Annotations[controllers.BootstrapTokenHashAnnotation]
...
`
	...
	Context("test rolling update with zone awareness disabled", Serial, Label("envtest", "dummy"), func() {
		...
		It("Update should correctly replace pods maxUnavailable=2", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 2, maxUnavailable.IntValue())
		...
		It("Update should correctly replace pods maxUnavailable=25%", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 2, 2) // nodeCount 9 * 25 % = 2.25 pods, rounded down is 2
		...
		It("Update should correctly replace pods maxUnavailable=50%", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 2, 4) // nodeCount 9 * 50 % = 4.50 pods, rounded down is 4
		...
		It("Update should correctly replace pods maxUnavailable=100%", func() {
			...
			ensurePodsRollingRestart(ctx, controllers.NewHumioNodeManagerFromHumioCluster(&updatedHumioCluster), 2, toCreate.Spec.NodeCount)
	...
	Context("Humio Cluster PodDisruptionBudgets", func() {
		It("Should correctly manage pod disruption budgets", func() {
			...
			// Test with default values (no PDB spec provided)
			suite.UsingClusterBy(key.Name, "Creating the cluster without PDB spec")
			Expect(k8sClient.Create(ctx, toCreate)).Should(Succeed())
			defer suite.CleanupCluster(ctx, k8sClient, toCreate)

			// Should not create a PDB when no spec is provided
			suite.UsingClusterBy(key.Name, "Verifying no PDB is created when no spec is provided")
			var pdb policyv1.PodDisruptionBudget
			Consistently(func() error {
				return k8sClient.Get(ctx, types.NamespacedName{
					Name:      fmt.Sprintf("%s%s", key.Name, key.Name),
					Namespace: key.Namespace,
				}, &pdb)
			}, time.Second*5, time.Second).Should(MatchError(k8serrors.IsNotFound))

			// Test with MinAvailable
			suite.UsingClusterBy(key.Name, "Updating cluster with MinAvailable PDB spec")
			var updatedHumioCluster humiov1alpha1.HumioCluster
			Eventually(func() error {
				err := k8sClient.Get(ctx, key, &updatedHumioCluster)
				if err != nil {
			...
			suite.UsingClusterBy(key.Name, "Verifying PDB is created with correct MinAvailable")
			Eventually(func() error {
				return k8sClient.Get(ctx, types.NamespacedName{
					Name:      fmt.Sprintf("%s%s", key.Name, key.Name),
					Namespace: key.Namespace,
			...
			Expect(pdb.Spec.MinAvailable).To(Equal(&minAvailable))
			Expect(pdb.Spec.MaxUnavailable).To(BeNil())
			Expect(pdb.Spec.Selector.MatchLabels).To(Equal(kubernetes.LabelsForHumio(key.Name)))

			// Test with MaxUnavailable
			suite.UsingClusterBy(key.Name, "Updating PDB to use MaxUnavailable")
			Eventually(func() error {
				err := k8sClient.Get(ctx, key, &updatedHumioCluster)
				if err != nil {
					return err
			...
			Eventually(func() error {
				...
				maxUnavailable := intstr.FromInt(1)
				if !reflect.DeepEqual(pdb.Spec.MaxUnavailable, &maxUnavailable) {
					return fmt.Errorf("maxUnavailable does not match expected value")
				}
				return nil
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// Test Node Pool PDBs
			suite.UsingClusterBy(key.Name, "Adding node pool with PDB")
			Eventually(func() error {
				err := k8sClient.Get(ctx, key, &updatedHumioCluster)
				if err != nil {
					return err
				}
				poolMaxUnavailable := intstr.FromInt(1)
				updatedHumioCluster.Spec.NodePools = []humiov1alpha1.HumioNodePoolSpec{
					{
						Name: "pool1",
						HumioNodeSpec: humiov1alpha1.HumioNodeSpec{
							NodeCount: 2,
							DataVolumePersistentVolumeClaimSpecTemplate: toCreate.Spec.DataVolumePersistentVolumeClaimSpecTemplate,
						},
						PodDisruptionBudget: &humiov1alpha1.HumioPodDisruptionBudgetSpec{
							MaxUnavailable: &poolMaxUnavailable,
						},
					},
				}
				return k8sClient.Update(ctx, &updatedHumioCluster)
			...
			suite.UsingClusterBy(key.Name, "Verifying node pool PDB is created")
			var nodePoolPdb policyv1.PodDisruptionBudget
			Eventually(func() error {
				return k8sClient.Get(ctx, types.NamespacedName{
					Name:      fmt.Sprintf("%s-pool1%s", key.Name, key.Name),
					Namespace: key.Namespace,
				}, &nodePoolPdb)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// Replace the manual label construction with GetNodePoolLabels()
			hnp := controllers.NewHumioNodeManagerFromHumioNodePool(&updatedHumioCluster, &updatedHumioCluster.Spec.NodePools[0])
			Expect(nodePoolPdb.Labels).To(Equal(hnp.GetNodePoolLabels()))

			maxUnavailable := intstr.FromInt(1)
			Expect(nodePoolPdb.Spec.MaxUnavailable).To(Equal(&maxUnavailable))

			// Test UnhealthyPodEvictionPolicy
			suite.UsingClusterBy(key.Name, "Updating node pool PDB with UnhealthyPodEvictionPolicy")
			Eventually(func() error {
				err := k8sClient.Get(ctx, key, &updatedHumioCluster)
				if err != nil {
					return err
				...
				updatedHumioCluster.Spec.NodePools[0].PodDisruptionBudget.UnhealthyPodEvictionPolicy = &policy
				return k8sClient.Update(ctx, &updatedHumioCluster)
			}, testTimeout, suite.TestInterval).Should(Succeed())

			suite.UsingClusterBy(key.Name, "Verifying UnhealthyPodEvictionPolicy is updated")
			Eventually(func() error {
				err := k8sClient.Get(ctx, types.NamespacedName{
					Name:      fmt.Sprintf("%s-pool1%s", key.Name, key.Name),
					Namespace: key.Namespace,
				...
				if nodePoolPdb.Spec.UnhealthyPodEvictionPolicy == nil || *nodePoolPdb.Spec.UnhealthyPodEvictionPolicy != policyv1.IfHealthyBudget {
					return fmt.Errorf("unhealthy pod eviction policy not set correctly")
				}
				return nil
			}, testTimeout, suite.TestInterval).Should(Succeed())

			// Test PDB cleanup
			suite.UsingClusterBy(key.Name, "Removing PDB configuration")
			Eventually(func() error {
				err := k8sClient.Get(ctx, key, &updatedHumioCluster)
				if err != nil {
					return err
				...
				updatedHumioCluster.Spec.PodDisruptionBudget = nil
				updatedHumioCluster.Spec.NodePools[0].PodDisruptionBudget = nil
				return k8sClient.Update(ctx, &updatedHumioCluster)
			}, testTimeout, suite.TestInterval).Should(Succeed())
			...
			// Test orphaned PDB cleanup
			suite.UsingClusterBy(key.Name, "Creating an orphaned PDB")
			orphanedPdb := &policyv1.PodDisruptionBudget{
				ObjectMeta: metav1.ObjectMeta{
					Name:      fmt.Sprintf("%s-orphaned%s", key.Name, key.Name),
					Namespace: key.Namespace,
					Labels:    kubernetes.LabelsForHumio(key.Name),
				},
				Spec: policyv1.PodDisruptionBudgetSpec{
					MinAvailable: &minAvailable,
					Selector: &metav1.LabelSelector{
						MatchLabels: kubernetes.LabelsForHumio(key.Name),
					},
				},
			}
