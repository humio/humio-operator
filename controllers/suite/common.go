	...
	if !helpers.UseEnvtest() && !helpers.UseDummyImage() {
		...
		if updatedHumioCluster.Spec.DisableInitContainer {
			Eventually(func() []string {
				...
				client := humioClient.GetHumioHttpClient(clusterConfig.Config(), reconcile.Request{NamespacedName: key})
				cluster, err := humioClient.GetClusters(ctx, client, reconcile.Request{NamespacedName: key})
				if err != nil {
